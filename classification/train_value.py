...
for epoch in range(Config.EPOCHS):  # 10 epochs for example
    ...
    print(epoch, "TRAIN", running_loss / len(train_dataloader))
    scheduler.step()
    ...
    for i, (inputs, targets) in enumerate(validation_dataloder):
        with torch.no_grad():
            ...
            loss = criterion(outputs, targets)
            running_loss += loss.item()
            # print(f"EPOCH: {epoch} ITERATION: {i+1} VALID LOSS: {loss.item()}")
    ...
    print(epoch, "VALID", running_loss / len(validation_dataloder))
